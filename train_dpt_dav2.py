"""
train_dpt_dav2.py — Train DPT decoder on frozen Depth Anything V2 encoder (local server)

DAv2 uses a DINOv2 backbone that was fine-tuned on 62M+ depth images.
Compared to vanilla DINOv2+DPT, the encoder already "understands" depth,
which may yield better depth (and possibly normal) predictions on tactile data.

DAv2 weights are auto-downloaded from HuggingFace on first run.

Usage:
  # Default: vitb (Base, same size as dinov2_vitb14)
  python train_dpt_dav2.py --device cuda:1

  # Use larger model:
  python train_dpt_dav2.py --dav2-size large --device cuda:1

  # Resume:
  python train_dpt_dav2.py --device cuda:1 --resume output_checkpoints/dpt_dav2/latest.pth
"""
import argparse
import os
...
from models.dpt import DAv2WithDPT


DAV2_CONFIGS = {
    "small": {"dinov2_model": "dinov2_vits14", "repo": "depth-anything/Depth-Anything-V2-Small", "file": "depth_anything_v2_vits.pth"},
    "base":  {"dinov2_model": "dinov2_vitb14", "repo": "depth-anything/Depth-Anything-V2-Base",  "file": "depth_anything_v2_vitb.pth"},
    "large": {"dinov2_model": "dinov2_vitl14", "repo": "depth-anything/Depth-Anything-V2-Large", "file": "depth_anything_v2_vitl.pth"},
}
...
def get_dav2_weights(size):
    """Auto-download DAv2 weights from HuggingFace, return local path."""
    from huggingface_hub import hf_hub_download
    cfg = DAV2_CONFIGS[size]
    path = hf_hub_download(cfg["repo"], cfg["file"])
    print(f"  DAv2 weights: {path}")
    return path
...
def plot_loss_curves(history, save_path):
    ...
    fig.suptitle("DPT DAv2 Training Curves", fontsize=14, fontweight="bold")
...
def parse_args():
    ...
    p.add_argument("--dav2-size", default="base", choices=["small", "base", "large"])
    p.add_argument("--dav2-weights", default=None,
                   help="Local .pth path; auto-downloaded from HuggingFace if not set")
...
def main():
    ...
    if args.save_path is None:
        args.save_path = f"output_checkpoints/dpt_dav2_{args.dav2_size}"
    ...
    cfg = DAV2_CONFIGS[args.dav2_size]
    dinov2_model = cfg["dinov2_model"]
    layer_indices = LAYER_INDICES[dinov2_model]

    # ── DAv2 weights ────────────────────────────────────────────────────────
    dav2_weights = args.dav2_weights or get_dav2_weights(args.dav2_size)
    ...
    # ── model ───────────────────────────────────────────────────────────────
    print(f"\nBuilding DAv2-{args.dav2_size} encoder ({dinov2_model})...")
    model = DAv2WithDPT(
        model_name=dinov2_model, weights=dav2_weights,
        features=args.dpt_features, layer_indices=layer_indices,
    ...
    print("=" * 60)
    print(f"DPT Training | encoder=DAv2-{args.dav2_size} ({dinov2_model})  epochs={args.epochs}")
